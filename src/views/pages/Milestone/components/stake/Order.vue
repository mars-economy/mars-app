<template>
  <div class="current-order p-py-3">
    <template v-if="isStakeSuccess()">
      ...
      <TextPair :color="'primary'" :data="yourTotalStake.amount + ' BUSD'" class="p-mt-2" icon="coins"
                label="your total stake"/>
      <TextPair :color="'primary'" :data="yourTotalStake.win + ' BUSD'" class="p-mt-2" icon="win"
                label="total estimated win"/>
      <Divider class="p-my-3" type="solid"/>
    </template>

    <CardLabel label="current order" labelPos="right"/>
    <TextPair :data="currentStake.amount + ' BUSD'" class="p-mt-2" icon="coins" label="your stake"/>
    <TextPair :data="calculateEstimatedWin() + ' BUSD'" class="p-mt-2" icon="win" label="estimated win"/>
